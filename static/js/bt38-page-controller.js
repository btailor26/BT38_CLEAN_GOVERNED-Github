...
(function () {
  ...
  if (root && root.dataset.bt38Page === "productLinking") {
    ...
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", revealServerRenderedProductLinking, {once: true});
    else revealServerRenderedProductLinking();
    ...
    window.BT38.PageController = {owner: "product-linking-session.js", skipped: true, serverRendered: true};
    return;
  }
  ...
  function currentRoot() { return document.querySelector("[data-bt38-page]"); }
  function pageState(name) { return window.BT38.pages[name] || null; }
  ...
  function updateCount(page, start, end) {
    ...
    if (status) status.textContent = `Page ${page.currentPage} of ${Math.max(1, Math.ceil(total / page.perPage))} · ${total} total`;
  }
  ...
  function render(name) {
    ...
    page.rows.forEach((row) => { row.el.hidden = !visibleRows.has(row); });
  ...
  function cacheRows(name) {
    ...
    page.rows = Array.from(table.querySelectorAll(page.rowSelector)).map((element) => ({el: element, text: lower(element.textContent), dataset: Object.assign({}, element.dataset)}));
  ...
  function wireForm(page) {
    const form = page.filterFormSelector ? document.querySelector(page.filterFormSelector) : null;
    if (!form) return;
    form.addEventListener("submit", (event) => { event.preventDefault(); event.stopPropagation(); filter(page.name, false); });
    form.querySelectorAll("input, select").forEach((field) => {
      const apply = (event) => { event.preventDefault(); event.stopPropagation(); filter(page.name, false); };
  ...
  function wirePagination(page) {
    ...
    const links = document.querySelectorAll(".bt38-page-nav .bt38-page-link");
    if (links[0]) links[0].addEventListener("click", (event) => { event.preventDefault(); if (page.currentPage > 1) { page.currentPage -= 1; render(page.name); } });
  ...
  function warehouseKpiCard(label) {
    return Array.from(document.querySelectorAll(".bt38-kpi-card")).find((card) => lower(card.querySelector("span")?.textContent) === lower(label));
  }
  ...
  async function updateWarehouseKpis() {
    try {
      ...
      const formatted = new Intl.NumberFormat("en-GB", {style: "currency", currency: "GBP", maximumFractionDigits: 0}).format(Number(data.inventory_value || 0));
      const missing = Number(data.missing_cogs_skus || 0);
      const costed = Number(data.costed_skus || 0);
      setWarehouseKpi("Inventory Value", formatted, missing ? `${costed} SKUs costed · ${missing} missing COGS` : `${costed} SKUs costed · stock × COGS`);
    } catch (error) { console.warn("[warehouse-kpi] aggregate read unavailable", error); }
  }
  async function updateWarehouseStatusKpi() {
    setWarehouseKpi("Warehouse Status", "Checking", "Runtime heartbeat");
    try {
      const response = await fetch("/governed/warehouse/runtime-state", {credentials: "include", headers: {Accept: "application/json"}, cache: "no-store"});
      const data = await response.json();
      const live = response.ok && data && data.ok === true;
      setWarehouseKpi("Warehouse Status", live ? "Live" : "Attention", live ? "Runtime healthy" : "Runtime check required");
    } catch (error) { setWarehouseKpi("Warehouse Status", "Attention", "Runtime unavailable"); }
  }
  function normalizeWarehouseUi() {
    if (!document.querySelector('[data-bt38-page="warehouse"]')) return;
    const routes = {"master stock": "/warehouse", "fba read only": "/fba-inventory", "group view": "/product-linking", "listings": "/warehouse?view=listings", "orders": "/orders", "stock transfer": "/warehouse?view=stock-transfer"};
  ...
  const controller = {
    register(name, config) {
      window.BT38.pages[name] = {name, filterFormSelector: config.filterFormSelector || null, tableSelector: config.tableSelector || null, rowSelector: config.rowSelector || "tbody tr", currentPage: 1, perPage: 15, rows: [], filteredRows: [], ready: false};
      return window.BT38.pages[name];
    },
    initTableCache: cacheRows,
    refreshTableCache(name) { const cached = cacheRows(name); if (cached) filter(name, true); return cached; },
    localFilter: filter,
    renderPage: render,
    autoRegisterFromDom() {
      const pageRoot = currentRoot();
      if (!pageRoot || !pageRoot.dataset.bt38Page) return false;
      const page = controller.register(pageRoot.dataset.bt38Page, {filterFormSelector: pageRoot.dataset.bt38FilterForm, tableSelector: pageRoot.dataset.bt38Table, rowSelector: pageRoot.dataset.bt38Row || "tbody tr"});
      const fbmSessionOwned = page.name === "fbm";
      if (pageRoot.dataset.bt38SubmitName && !fbmSessionOwned) {
        window[pageRoot.dataset.bt38SubmitName] = function (event) { if (event) { event.preventDefault(); event.stopPropagation(); } return filter(page.name, false); };
      }
      // FBM follows the established Warehouse single-owner contract. Its
      // session/lifecycle controller owns history, lifecycle and search; this
      // shared controller only presents that committed subset through paging.
      // Wiring the FBM form here would create a second filter authority.
      if (!fbmSessionOwned) wireForm(page);
      wirePagination(page);
      if (cacheRows(page.name)) {
        if (fbmSessionOwned && typeof window.BT38FBMApplyCommittedSnapshot === "function") window.BT38FBMApplyCommittedSnapshot();
        else if (!fbmSessionOwned) filter(page.name, false);
      }
      return true;
    }
  };
  ...
  window.bt38SetFilter = function () { const pageRoot = currentRoot(); return pageRoot ? filter(pageRoot.dataset.bt38Page, false) : false; };
  document.addEventListener("DOMContentLoaded", () => { controller.autoRegisterFromDom(); normalizeWarehouseUi(); });
}());
