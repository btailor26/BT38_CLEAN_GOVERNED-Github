...
def api_diagnostics_ebay_health():
    """eBay health check using GeteBayOfficialTime API"""
    if request.headers.get("X-Task-Key") != os.getenv("TASK_API_KEY"):
        return jsonify({"error": "unauthorized"}), 401
    ...
    try:
        ebay_store = Store.query.filter_by(platform='eBay', is_active=True).first()
        if not ebay_store:
            return jsonify({"ok": False, "error": "No active eBay store found"}), 404
        
        ebay_svc = eBayAPIService()
        success, time_or_error = ebay_svc.get_ebay_official_time(ebay_store)
        
        if success:
            return jsonify({"ok": True, "time": time_or_error, "store": ebay_store.name})
        else:
            return jsonify({"ok": False, "error": time_or_error}), 500
            
    except Exception as e:
        logging.error(f"Error checking eBay health: {str(e)}")
        return jsonify({"ok": False, "error": str(e)}), 500

@bp.get("/api/diagnostics/ebay/policies")
def api_diagnostics_ebay_policies():
    """Get eBay Business Policies (Shipping, Payment, Return)"""
    if request.headers.get("X-Task-Key") != os.getenv("TASK_API_KEY"):
        return jsonify({"error": "unauthorized"}), 401
    ...
    try:
        ...
        ebay_svc = eBayAPIService()
        success, profiles_or_error = ebay_svc.get_seller_profiles(ebay_store)
        
        if success:
            return jsonify({
                "ok": True,
                "store": ebay_store.name,
                "profiles": profiles_or_error
            })
        else:
            return jsonify({"ok": False, **profiles_or_error}), 500
            
    except Exception as e:
        logging.error(f"Error fetching eBay policies: {str(e)}")
        return jsonify({"ok": False, "error": str(e)}), 500

@bp.get("/api/diagnostics/ebay/raw-import")
def api_diagnostics_ebay_raw_import():
    """Test eBay import and return raw API response for debugging"""
    if request.headers.get("X-Task-Key") != os.getenv("TASK_API_KEY"):
        return jsonify({"error": "unauthorized"}), 401
    ...
    try:
        ebay_store = Store.query.filter_by(platform='eBay', is_active=True).first()
        if not ebay_store:
            return jsonify({"error": "No active eBay store found"})
        
        ebay_svc = eBayAPIService()
        success, items, message = ebay_svc.import_inventory_from_ebay(ebay_store)
        
        return jsonify({
            "success": success,
            "items_count": len(items) if items else 0,
            "message": message,
            "first_5_items": items[:5] if items else [],
            "store_name": ebay_store.name,
            "api_key_length": len(ebay_store.api_key) if ebay_store.api_key else 0
        })
    except Exception as e:
        import traceback
        return jsonify({
...
def admin_fix_sandbox_flag():
    """Fix production store credentials to include sandbox:false flag"""
    if request.headers.get("X-Task-Key") != os.getenv("TASK_API_KEY"):
        return jsonify({"error": "unauthorized"}), 401
    ...
    try:
        ...
        ebay_store = Store.query.filter_by(platform='eBay', is_active=True).first()
        if not ebay_store:
            return jsonify({"error": "No active eBay store found"}), 404
        
        creds = json_module.loads(ebay_store.api_key)
        before_sandbox = creds.get('sandbox', 'MISSING')
        creds['sandbox'] = False
        ebay_store.api_key = json_module.dumps(creds)
        db.session.commit()
        
        return jsonify({
            "success": True,
            "store": ebay_store.name,
            "before_sandbox": before_sandbox,
            "after_sandbox": False,
            "message": "Updated credentials to use live eBay API (sandbox=false)"
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def admin_ebay_normalize_itemids():
    """Normalize eBay MarketplaceListing.external_listing_id to numeric ItemIDs"""
    import re
    ...
    try:
        store = Store.query.filter_by(platform="eBay", is_active=True).first()
        if not store:
            return jsonify({"ok": False, "error": "No active eBay store found"}), 404

        svc = eBayAPIService()
        non_numeric = (MarketplaceListing.query
        ...
        for ml in non_numeric:
            scanned += 1
            item_id = (ml.external_listing_id or "").strip()
            if numeric_re.match(item_id):
                skipped += 1
                continue

            # Prefer explicit external_sku, else fallback to warehouse SKU
            sku_key = ml.external_sku or (ml.warehouse_stock.sku if ml.warehouse_stock else None)
            if not sku_key:
                skipped += 1
                continue

            try:
                resolved = svc.resolve_item_id_by_sku(sku_key, store)
                if resolved and numeric_re.match(str(resolved)):
                    # Check if this ItemID would create a duplicate
                    existing_with_id = MarketplaceListing.query.filter_by(
                        store_id=store.id,
                        external_listing_id=str(resolved)
                    ).first()
                    
                    if existing_with_id and existing_with_id.id != ml.id:
                        # Duplicate - skip to avoid constraint violation
                        skipped += 1
                        logging.warning(f"eBay repair: Skipping {sku_key} - ItemID {resolved} already exists")
                    else:
                        old = ml.external_listing_id
                        ml.external_listing_id = str(resolved)
                        db.session.add(ml)
                        fixed += 1
                        logging.info(f"eBay repair: {sku_key}: {old} -> {resolved}")
                else:
                    skipped += 1
            except Exception as ex:
                errors.append({"sku": sku_key, "error": str(ex)})

        db.session.commit()
        return jsonify({"ok": True, "scanned": scanned, "fixed": fixed, "skipped": skipped, "errors": errors}), 200
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error in eBay ItemID normalization: {str(e)}")
        return jsonify({"ok": False, "error": str(e)}), 500
...
@bp.route('/ebay-setup', methods=['GET', 'POST'])
def ebay_setup():
    """eBay API setup page"""
    if request.method == 'GET':
        return render_template('ebay_setup.html')
    
    # Handle form submission
    try:
        # Prefer Fly/env secrets; fall back to form for local manual testing
        app_id = (
            os.environ.get("EBAY_CLIENT_ID")
            or os.environ.get("EBAY_APP_ID")
            or request.form.get('app_id')
        )
        cert_id = (
            os.environ.get("EBAY_CLIENT_SECRET")
            or os.environ.get("EBAY_CERT_ID")
            or request.form.get('cert_id')
        )
        dev_id = request.form.get('dev_id')
        access_token = request.form.get('access_token')
        refresh_token = request.form.get('refresh_token', '')
        site_id = request.form.get('site_id', '0')
        ...
        # Find or create beatsoutlet store
        store = db.session.query(Store).filter_by(name='beatsoutlet').first()
        if not store:
            store = Store()
            store.name = 'beatsoutlet'
            store.platform = 'eBay'
        ...
        is_valid, validation_msg = ebay_service.validate_credentials_format(store.api_key)
        if not is_valid:
            flash(f"Invalid credentials format: {validation_msg}", 'error')
            return render_template('ebay_setup.html')
        
        # Save credentials first (before authentication test)
        db.session.commit()
        
        # Test authentication (credentials already saved)
        connection_success = ebay_service.authenticate_store(store)
        if connection_success:
            ...
            store.sync_status = 'pending'
            db.session.commit()
            flash(" eBay credentials saved! Connection test failed - this is normal for sandbox. Your store is ready and will sync when tokens are available.", 'info')
            return redirect(url_for('routes.stores'))
            
    except Exception as e:
        logging.error(f"Error setting up eBay store: {str(e)}")
        flash(f"Error: {str(e)}", 'error')
        return render_template('ebay_setup.html')

@bp.route('/test-ebay-connection', methods=['POST'])
def test_ebay_connection_simple():
    """Test eBay connection endpoint for AJAX calls"""
    try:
        data = request.get_json()
        ...
        # Validate credentials format
        is_valid, validation_msg = ebay_service.validate_credentials_format(credentials_json)
        if not is_valid:
            return jsonify({'success': False, 'error': f'Invalid credentials format: {validation_msg}'})
        
        # For now, just validate the format since we don't need full authentication without access token
        return jsonify({'success': True, 'message': 'Credentials format is valid!'})
...
def quick_setup_amazon():
    ...
    try:
        ...
        from amazon_service import AmazonAPIService
        amazon_service = AmazonAPIService()
        auth_success = amazon_service.authenticate_store(store)
            ...
            if store.platform.lower() in ['amazon', 'ebay'] and store.api_key.strip():
                try:
                    ...
                    if service:
                        ...
                        if is_valid:
                            connection_success = service.authenticate_store(store)
                            if connection_success:
                                store.sync_status = 'success'
                                store.last_sync = datetime.utcnow()
...
def test_ebay_connection(store_id):
    ...
    try:
        ...
        auth_success = ebay_service.authenticate_store(store)
...
def amazon_credentials_setup():
    ...
    if request.method == 'POST':
        try:
            ...
            from amazon_service import AmazonAPIService
            amazon_service = AmazonAPIService()
            success, items, message = amazon_service.import_inventory_from_amazon(amazon_store)
