...
def test_customer_account_owns_branding_five_seats_and_role_presets():
    ...
    assert "if_member_count(a.id)>=int(a.user_limitor5):" in compact
    ...
    # Billing is an owner-only workspace surface reached from Profile, not a second user/subscription authority.
    assert '@app.get("/billing")' in profile_service
    assert 'href="/billing"' in profile_page
...
def test_customer_shell_only_rebrands_after_customer_logo_exists():
    ...
    compact_shell = _compact(shell_block)
    assert "ifaanda.logo_data:" in compact_shell
    assert "BT38 Inventory" in shell_block
    assert "Powered by BT38" in shell_block
    assert 'href="/profile"' in shell_block
    assert 'href="/billing"' not in shell_block
    assert 'href="/billing"' in profile_page
...
def test_first_login_hands_once_to_cofi_dashboard_welcome():
    ...
    assert 'session["bt38_cofi_first_welcome"]=name' in compact
