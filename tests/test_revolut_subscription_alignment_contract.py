...
BILLING_COMPACT = _compact(BILLING)
...
def test_revolut_binding_extends_existing_assignment_not_customer_authority():
    ...
    assert "assignment.provider_subscription_ref=binding.subscription_ref" in SERVICE_COMPACT
    assert "assignment.provider_subscription_ref=subref" in SERVICE_COMPACT
    assert 'account.billing_status="setup_pending"' in SERVICE_COMPACT
...
def test_paid_setup_requires_exact_revolut_plan_variation_and_owner_action():
    ...
    assert 'package.tier_type!="paid"' in SERVICE_COMPACT
    assert 'assignment.billing_provider!="revolut"' in SERVICE_COMPACT
    assert "package.revolut_plan_ref" in SERVICE
    assert "plan_variation_id=str(package.revolut_plan_ref).strip()" in SERVICE_COMPACT
    assert 'idempotency_key=f"bt38-account-{account.id}-package-{package.id}"' in SERVICE_COMPACT


def test_provider_customer_and_subscription_identity_are_persisted_before_reuse():
    customer_commit = SERVICE_COMPACT.index("binding.customer_ref=ref")
    customer_commit_end = SERVICE_COMPACT.index("db.session.commit()", customer_commit)
    create_subscription = SERVICE_COMPACT.index("client.create_subscription(")
    assert customer_commit < customer_commit_end < create_subscription
    assert "ifbinding.subscription_ref:" in SERVICE_COMPACT
    assert "client.retrieve_subscription(binding.subscription_ref)" in SERVICE_COMPACT
...
def test_signed_webhook_resolves_exact_order_subscription_for_setup_and_recurring_cycles():
    ...
    assert "order=client.retrieve_order(order_ref)" in SERVICE_COMPACT
    assert 'sd.get("subscription_id")' in SERVICE_COMPACT
    assert "query.filter_by(subscription_ref=subref)" in SERVICE_COMPACT
    assert 'assignment.billing_provider!="revolut"' in SERVICE_COMPACT
    readback = SERVICE_COMPACT.index("subscription=client.retrieve_subscription(binding.subscription_ref)")
    mutation = SERVICE_COMPACT.index("_sync_assignment(assignment,binding,subscription)", readback)
    assert readback < mutation
    assert "subscriptionidentitydidnotmatch" in SERVICE_COMPACT.lower()
    assert 'return("",204)' in SERVICE_COMPACT


def test_revolut_webhook_fails_closed_until_signing_secret_is_in_fly():
    assert 'reason":"webhook_not_configured"' in SERVICE_COMPACT
    assert "exceptRevolutBillingError:" in SERVICE_COMPACT
    assert "verify_revolut_webhook_signature(" in SERVICE
...
def test_admin_webhook_provisioning_is_explicit_and_secret_is_not_persisted():
    ...
    assert "ifnot_is_admin():" in SERVICE_COMPACT
    assert "client.list_webhooks()" in SERVICE_COMPACT
    assert "client.retrieve_webhook(wid)" in SERVICE_COMPACT
    assert "client.create_webhook(url=target,events=list(_WEBHOOK_EVENTS))" in SERVICE_COMPACT
    assert '["ORDER_AUTHORISED","ORDER_COMPLETED","ORDER_CANCELLED"]' in SERVICE_COMPACT
...
def test_revolut_subscription_states_map_to_existing_bt38_billing_statuses():
    assert 'ifstate=="active":return"active"' in SERVICE_COMPACT
    assert 'ifstatein{"overdue","paused"}:return"past_due"' in SERVICE_COMPACT
    assert 'ifstatein{"cancelled","finished"}:return"cancelled"' in SERVICE_COMPACT
    assert 'return"setup_pending"' in SERVICE_COMPACT


def test_billing_page_uses_hosted_revolut_setup_not_local_card_capture():
    assert 'action="/billing/revolut/start"' in BILLING
    assert "Continuetopayment" in BILLING_COMPACT
    assert "BT38doesnotstorecard,bankorwalletcredentials." in BILLING_COMPACT
