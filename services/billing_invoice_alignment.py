...
Payment-provider references remain internal audit evidence only. BT38 renders its
own customer PDF/CSV documents without exposing the processor or calling a paid
...
class BillingInvoice(db.Model):
    ...
    invoice_number = db.Column(db.String(80), nullable=False, unique=True, index=True)
...
def record_completed_revolut_invoice(*, account, assignment, package, order: dict, subscription_ref: str) -> BillingInvoice | None:
    """Persist one invoice per exact completed provider order; duplicate webhooks are idempotent."""
    ...
    if not order_ref or len(currency) != 3:
        raise ValueError("Completed payment is missing invoice authority fields")
    ...
    invoice = BillingInvoice(
        ...
        invoice_number=f"BT38-{datetime.utcnow().strftime('%Y%m%d')}-{order_ref[-8:].upper()}",
...
def _pdf_bytes(invoice: BillingInvoice) -> bytes:
    """Small dependency-free customer PDF generated by BT38 itself."""
    lines = [
        "BT38 PAID INVOICE",
...
def bt38_billing_invoice_csv(invoice_id: int):
    ...
    writer.writerow(["invoice_number", "paid_at", "business_name", "package", "amount", "currency", "status"])
    writer.writerow([invoice.invoice_number, invoice.paid_at.isoformat(), invoice.business_name, invoice.package_name, f"{invoice.amount_minor / 100:.2f}", invoice.currency, "Paid"])
