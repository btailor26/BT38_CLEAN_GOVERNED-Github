...
def bt38_customer_owned_shell_alignment(response):
 ...
 # Billing is intentionally owned by the Profile/workspace surface, not the top navigation account menu.
 top='''<a class="nav-link text-light" href="/login">\n                    <i data-feather="log-in" class="me-1"></i>Login\n                </a>''';menu=f'''<div class="dropdown"><button class="btn btn-dark border-0 dropdown-toggle d-flex align-items-center gap-2" type="button" data-bs-toggle="dropdown"><span class="rounded-circle bg-light text-dark d-inline-flex align-items-center justify-content-center" style="width:30px;height:30px;font-weight:700">{html.escape(display[:1].upper())}</span><span class="d-none d-md-inline text-start"><span class="d-block">{display}</span>{f'<small class="text-secondary">{position}</small>' if position else ''}</span></button><ul class="dropdown-menu dropdown-menu-end shadow-sm"><li><a class="dropdown-item" href="/profile"><i data-feather="user" class="me-2"></i>Profile</a></li><li><hr class="dropdown-divider"></li><li><a class="dropdown-item" href="/logout"><i data-feather="log-out" class="me-2"></i>Sign out</a></li></ul></div>''';body=body.replace(top,menu,1);response.set_data(body);return response
