...
(function(){{
  ...
  // existing committed marketplace event handoff; with no event the page sleeps.
  function applyHistory(){{currentPage=1;range=String(rangeInput&&rangeInput.value||'3d').toLowerCase();from=String(fromInput&&fromInput.value||'');to=String(toInput&&toInput.value||'');saveSession();render();}}
